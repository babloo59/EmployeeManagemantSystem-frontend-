...
function ManageUsers() {
  ...
  const loadUsers = async () => {
    try {
      setLoading(true);
      const res = await api.get("/manager/view-employees");
      setUsers(res.data);
    } catch {
  ...
  const toggleStatus = async (id, currentStatus) => {
    try {
      ...
      await api.put(`/manager/employee/status/${id}?userStatus=${newStatus}`);
      loadUsers();
    } catch {
    ...
    <div className="min-vh-100 bg-light py-5">
      <Container>
        ...
        <div className="d-flex justify-content-end align-items-center mb-3">
          <Button onClick={() => navigate("/manager/add-user")}>
